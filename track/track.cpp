...
public:
	...
	virtual void Process(IplImage* pImg)
	{
        ...
        morphologyEx(Mat(mask), Mat(mask), MORPH_OPEN, getStructuringElement(MORPH_ELLIPSE, Size(3, 3)));
...
enum FRAME_STATUS { no, yes, maybe, candidate };
struct FrameInfo
{
    float v;
...
public:
    ...
    FRAME_STATUS update(FrameInfo _v, Mat frame)
    {
        list.add(_v);
        ++nFrame;
        
        const unsigned long cFrame = list.getTail()-1;
        const unsigned long maxHistory = fps * 3;
        const unsigned long maxDetectGap = fps * 1;
        const unsigned long maxBackgroundCount = 5;
        FrameInfo& v = list.get(cFrame);
        const float candidateThre = 0.02;
        if(list.getSize() > 1)
        {
            FrameInfo& pv = list.get(cFrame-1);
            float mean_v = (v.v+pv.v)*0.5;
            if(mean_v > candidateThre)
            {
                nBackground = 0;
                if(mean_v >= pv.v)
                {
            ...
            {
                if(lastDetectEnd==nFrame-2)
                {
                    lastDetectEnd = nFrame - 1;
                    nBackground = 1;
                }
                else
                    ++nBackground;
        ...
        if(nBackground > maxBackgroundCount || nFrame-lastDetectEnd>maxHistory)
        {
            unsigned long detectedFrame = -1;
            for(unsigned long i=lastDetectEnd; i<nFrame; ++i)
            {
                FrameInfo& hv = list.get(i);
                if(hv.fstatus==candidate)
                {
                    if(detectedFrame == -1)
                    {
                        detectedFrame = i;
                    ...
                    {
                        FrameInfo& dv = list.get(detectedFrame);
                        if(hv.v > dv.v)
                        {
                            dv.fstatus = maybe;
                            dv.frame.release();
            ...
            {
                lastDetectEnd = max(detectedFrame-maxDetectGap, nFrame-nBackground);
            }
            if(detectedFrame != -1)
            {
                char savepath[1024];
                sprintf(savepath, "%s/%ld.jpg", "/Users/sunkai/Downloads/测试视频/snappic", detectedFrame);
                imwrite(savepath, list.get(detectedFrame).frame);
                imshow("snap", list.get(detectedFrame).frame);
            }
        }
        return v.fstatus;
    }
    ...
    void show(Mat& img)
    {
        ...
        for(unsigned long k=list.getHead(); k<list.getTail(); ++k)
        {
            ...
            else if(v.fstatus==candidate)
            {
                circle(img, p2, 2, Scalar(255), 1);
            }
            p1 = p2;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    namedWindow("info", CV_WINDOW_NORMAL);
    namedWindow("snap", CV_WINDOW_NORMAL);
#endif
    ...
    char c;
    while (!frame.empty() && (c=waitKey(30))!=27)
    {
        if(c=='b')
        {
            while(waitKey(1000)!='c');
        }
        Size nsize;
        ...
        Rect roiRect(0, nsize.height*0.5-windowSize*0.5, nsize.width, windowSize);
        ...
        FRAME_STATUS status = detector.update(v, frame);
