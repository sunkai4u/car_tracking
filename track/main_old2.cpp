        ...
        if(list.getSize() > maxHistory)
        {
            ...
            if(v.v > candidateThre)
            {
                if(v.v >= pv.v)
                {
                    v.vstatus = up;
                    v.fstatus = maybe;
                    FrameInfo& cv = list.get(candidateFrame);
                    if(cv.fstatus==candidate)
                        cv.fstatus = maybe;
                    candidateFrame = nFrame - 1;
                    candidateMat = frame.clone();
                }
                else
                {
                    v.vstatus = down;
                    ...
                    unsigned int maybeCount = 0;
                    const unsigned int maxNochangeCount = 3;
                    const unsigned int maxMaybeCount = fps*0.5;
                    ...
                    if(i==maxHistory || noChangeCount>maxNochangeCount || maybeCount>maxMaybeCount)
                    {
                        list.get(candidateFrame).fstatus = yes;
                    }
                }
            }
            else
